...
from alasso.utils import extract_weight_changes, compute_updates
from alasso.regularizers import quadratic_regularizer
...
class KOOptimizer(Optimizer):
    ...
    def __init__(self, opt, step_updates=[], task_updates=[], init_updates=[], task_metrics = {}, regularizer_fn=quadratic_regularizer,
                c_param=1.0, model=None, compute_average_loss=False, compute_average_weights=False, **kwargs):
        ...
        Args:
            ...
            c_param: init value of c in the paper
        ...
        self.c_param = K.variable(value=c_param, dtype=tf.float32, name="c_param")
        self.c_prime = K.variable(value=0.0, dtype=tf.float32, name="c_prime")
        self.nb_data = K.variable(value=1.0, dtype=tf.float32, name="nb_data")
        self.batch_size = K.variable(value=1.0, dtype=tf.float32, name="batch_size")
        self.opt = opt
        self.model = model
    ...
    def set_c(self, val):
        K.set_value(self.c_param, val)

    def set_c_prime(self, val):
        K.set_value(self.c_prime, val)

    def set_nb_data(self, nb):
        K.set_value(self.nb_data, nb)

    def set_batch_size(self, bs):
        K.set_value(self.batch_size, bs)
    ...
    def get_updates(self, weights, constraints, initial_loss, model=None):
        ...
        self.loss = initial_loss + self.c_param * self.regularizer
        ...
        # Keep batch size handy for normalization purposes
        self.vars['batch_size'] = self.batch_size
        ...
        for name, metric_fn in self.task_metrics.items():
            ...
            for w in weights:
                reset_ops.append(tf.assign(self.vars[name][w], tf.zeros_like(self.vars[name][w])))
                update_ops.append(tf.assign_add(self.vars[name][w], metric[w], use_locking=True))
    ...
    def update_task_metrics_start(self):
        # Reset metric accumulators
        sess = K.get_session()
        sess.run(self._reset_task_metrics_op)

    def update_task_metrics_batch(self, xi_original, yi_original, batch_size):
        sess = K.get_session()
        ...
        xi, yi, sample_weights = model._standardize_user_data(xi_original, yi_original, batch_size=batch_size)
        sess.run(self._update_task_metrics_op, {self.model.input:xi[0], model.targets[0]:yi[0], model.sample_weights[0]:sample_weights[0], K.learning_phase(): 0})

    def update_task_metrics(self, X, y, batch_size):
        self.update_task_metrics_start()

        n_batch = len(X) // batch_size
        for i in range(n_batch):
            self.update_task_metrics_batch(X[i * batch_size:(i+1) * batch_size], y[i*batch_size:(i+1)*batch_size], batch_size)
